...
const groupHD = (data) => {
  ...
  data.forEach((item) => {
    ...
    const existingThuoc = ketQua[key].THUOC.find(
      (thuoc) => thuoc.MATHUOC === item.MATHUOC
    );
    if (!existingThuoc) {
    ...
    const existingDichVu = ketQua[key].DICHVU.find(
      (dichvu) => dichvu.MADV === item.MADV
    );
    if (!existingDichVu) {
...
const groupTableLich = (data) => {
  ...
  Object.keys(ketQua).forEach((ngay) => {
    ...
    ketQua[ngay].sort(
      (ca1, ca2) => allMaca.indexOf(ca1.MACA) - allMaca.indexOf(ca2.MACA)
    );
  });
...
const mergeLich = (lichChuaDat, lichDaDat) => {
  ...
  lichChuaDat.forEach((lichNgayChuaDat) => {
    ...
    const lichNgayDaDat = lichDaDat.find(
      (lichNgay) => lichNgay.NGAY === lichNgayChuaDat.NGAY
    );
  ...
  lichDaDat.forEach((lichNgayDaDat) => {
    const ngayChuaDat = mergedLich.find(
      (lichNgay) => lichNgay.NGAY === lichNgayDaDat.NGAY
    );
...
const mergeCa = (caChuaDat, caDaDat) => {
  ...
  caDaDat.forEach((caDaDatItem) => {
    const existingCa = mergedCa.find(
      (caChuaDatItem) => caChuaDatItem.MACA === caDaDatItem.MACA
    );
