...
-- 1. TẠO CƠ SỞ DỮ LIỆU
-- 2. PROC(ĐÃ ĐƯỢC XỬ LÍ TRANH CHẤP)
-- 3. PHÂN QUYỀN
-- 4. NHẬP LIỆU
...
BEGIN TRAN
	BEGIN TRY
		...
		IF (NOT EXISTS(SELECT * FROM NHANVIEN WITH (UPDLOCK) WHERE MANV = @MANV))
		BEGIN
...
BEGIN TRAN
	BEGIN TRY
		-- Kiểm tra tồn tại tài khoản
		IF (NOT EXISTS(SELECT * FROM QTV WITH (UPDLOCK) WHERE MAQTV = @MAQTV))
		BEGIN
