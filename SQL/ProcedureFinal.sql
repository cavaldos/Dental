...
BEGIN TRAN
	BEGIN TRY
		...
		BEGIN
			...
			SELECT KH.HOTEN HOTENKH, HD.SODT SODT, HD.SOTT SOTTHD, NGAYXUAT, TONGCHIPHI, NV.MANV MANV, NV.HOTEN HOTENNV, _DATHANHTOAN, CTDV.MADV, TENDV, CTDV.SOLUONG SLDV, CTDV.DONGIALUCTHEM DONGIADV, CTT.MATHUOC, TENTHUOC, CTT.SOLUONG SLTHUOC, CTT.DONGIALUCTHEM DONGIATHUOC
			FROM HOADON HD
...
BEGIN TRAN
	BEGIN TRY
		...
		ELSE
		BEGIN
			SELECT KH.HOTEN HOTENKH, HD.SODT SODT, HD.SOTT SOTTHD, NGAYXUAT, TONGCHIPHI, NV.HOTEN HOTENNV, LDV.MADV MADV, CTDV.SOLUONG SLDV, CTDV.DONGIALUCTHEM DONGIADV, CTT.MATHUOC,TENTHUOC, CTT.SOLUONG SLTHUOC, CTT.DONGIALUCTHEM DONGIATHUOC
			FROM HOADON HD
			JOIN KHACHHANG KH ON HD.SODT = KH.SODT
			JOIN NHANVIEN NV ON NV.MANV = HD.MANV
			JOIN CHITIETDV CTDV ON CTDV.SODT = HD.SODT AND CTDV.SOTT = HD.SOTT
